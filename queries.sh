...
echo "$($PSQL "SELECT t.name AS winner FROM teams t LEFT JOIN games g ON t.team_id = g.winner_id WHERE g.year = 2018 AND round ILIKE 'final'")"
...
echo "$($PSQL "SELECT g.year, t.name FROM games g LEFT JOIN teams t ON t.team_id = g.winner_id WHERE round ILIKE 'final' ORDER BY g.year")"
